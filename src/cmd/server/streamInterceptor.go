...
func myStreamServerInterceptor1(src interface{}, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
	log.Println("[pre stream] my stream server interceptor 1: ", info.FullMethod)
	err := handler(src, &myServerStreamWrapper1{ss})
	log.Println("[post message] my stream server interceptor 1: ")
	return err
}
